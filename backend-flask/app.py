...
#CHECK Constraints
#NEED QUERIES to provide initial data
...
def register_student():
  ...
  if request.method == 'POST':
    ...
    length = len(email)
    if length < 5 or length > 25 or not(re.match(r'\w+@\w+',email)):
      return "email" 
    # print(email)
...
def register_employee():
  ...
  if request.method == 'POST':
    ...
    print(emp_email)
    length = len(emp_email)
    if length < 5 or length > 25 or not(re.match(r'\w+@\w+',emp_email)):
      return "email"
    ...
    print(emp_password)
    if len(emp_password) < 8:
      return 'password'
    conf_emp_password = request.form['conf_emp_password']
    if conf_emp_password != emp_password:
      return 'confirm password'
    ...
    try:
      ...
      cur = mysql.connection.cursor()
      cur.execute(f"select md5({emp_password})")
      emp_password = cur.fetchone() 
...
def login():
  ...
  if request.method == 'POST':
    ...
    try:
      ...
      cur.execute(f"select md5({password})")
      password = cur.fetchone() 
      cur.execute(f"select * from User where username = '{user}' and user_password = '{password}'")
      value = cur.fetchone()
    except Exception as e:
      print(e)
      return "Failed"
      ...
      if value:
        ...
        username = user
        global role
        cur.execute(f"select * from Student where student_username = '{user}' ")
        is_student = cur.fetchone()
        if is_student:
          role = "student"   
        cur.execute(f"select * from Employee where emp_username = '{user}' ")
        is_employee = cur.fetchone()
        if is_employee:
          role = "employee"
          global job
          cur.execute(f"select * from Labtech where labtech_username = '{user}' ")
          is_tech = cur.fetchone()
          if is_tech:
            job = "lab_technician"
            cur.execute(f"select * from Sitetester where sitetester_username = '{user}' ")
            is_tester = cur.fetchone()
            if is_tester:
              job = "both"
          else:
            job = "site_tester"
        cur.execute(f"select * from Administrator where admin_username = '{user}' ")
...
def test_sign_up():
  ...
  if request.method == 'POST':
    ...
    try:
      ...
      cur.execute(f"select * from appointment where username = '{username}' and site_name = '{site}' 
...
def tests_processed():
  if job != "lab technician" or job != "both":
    return "Invalid User Type"
  ...
  if request.method == 'POST':
    ...
    try:
      ...
      if len(test_ids) > 1:
        ...
        cur.execute(f"select * from test where pool_id = '{pool_id}' and test_id = '{test_id1}'")
        value = cur.fetchone()
        if value:
          return "Success"
        else:
          return "Failed"
      cur.execute(f"select count(*) from test where pool_id = '{pool_id}'")
      value = cur.fetchone()
      if value[0] == i:
        return "Success"
      # print("return value: ", value)
...
# localhost:5000/api/process_pool : POST
@app.route('/api/process_pool', methods=['GET', 'POST'])
def create_pool():
  if job != "lab technician" or job != "both":
    return "Invalid User Type"
  ...
  if request.method == 'POST':
    ...
    try:
      cur = mysql.connection.cursor()
      cur.execute(f"select max(appt_date) from test where pool_id = {pool_id}")
      late_date = cur.fetchone[0]
      if late_date >= date:
        return "Invalid Date"
      cur.callproc('process_pool', [pool_id, status, date, processed_by])
    except Exception as e:
      print(e)
      return "Failed"
    else:
      cur.execute(f"select * from pool where pool_id = '{pool_id}' and pool_status = '{status}' 
      and process_date = '{date}'and processed_by = '{processed_by}'")
      value = cur.fetchone()
      if value:
        return "Success"
      else:
